...
<div class="popup-form" v-show="popupVisibleRef">
    ...
    <div class="popup-form-title">
        <h2 class="headerpopup">Create Page Post</h2>
...
const submitForm = async () => {
    ...
    if (errors.value.length == 0){
        await axios
        .post(`/api/pages/${route.params.id}/createpost/`, formData, {
            headers: {
                "Content-Type": "multipart/form-data",
            }
        })
        .then(response => {
            console.log('page post data', response.data)
            page.posts.unshift(response.data.post)
            body.value = ''
            fileInputRef.value = null
            videoInputRef.value = null
            popupVisibleRef.value = !popupVisibleRef.value
...
/* .postform-top .postform-form .picture input{
...
/* .postform-top .postform-form .addToPost .images i {
font-size: 20px;
color: gray;
} */
