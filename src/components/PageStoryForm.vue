<template>
    ...
    <div class="popup-form" v-show="popupVisibleRefStory">
...
import { useToast } from 'vue-toast-notification';
    ...
    let popupVisibleRefStory = ref(false);

    const showPopup = () => {
        popupVisibleRefStory.value = !popupVisibleRefStory.value
    }
    ...
    const submitStory = async () => {
        ...
        if (errors.value.length == 0){
            await axios
            .post(`/api/pages/${route.params.id}/createstory/`, formData, {
                headers: {
                    "Content-Type": "multipart/form-data",
                }
            })
            .then(response => {
                console.log('page story data', response.data)
                toast.success("Story added")
                stories.value.unshift(response.data.story)
                body.value = ''
                fileInputRef.value = null
                videoInputRef.value = null
                popupVisibleRefStory.value = !popupVisibleRefStory.value
